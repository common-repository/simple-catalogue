...
class SimpleCatalogue {
	...
	public function __construct()
	{
		...
		//Creation des tables a l'installation du plugin
		register_activation_hook( __FILE__, array( 'SimpleCatalogue', 'sc_install' ) );
...
// Ajout d'un lien de menu dans l'admin de WP
	public function sc_add_admin_menu() {
		add_menu_page(__('Manage catalog', 'simple-catalogue') , 'Simple Catalogue' , 'manage_options' , 'catalogue' , array($this, 'menu_settings'), 'dashicons-index-card', '30');
...
// Creation de la page Ajout d'un element
	...
	public function menu_settings()
	{
		...
		echo '<div class="wrap" style="height: 67px;"><div id="col-left" style="width:50%; float: left"><h1>' . get_admin_page_title() . '</h1>' . __('You can add or modify items and categories', 'simple-catalogue') . '<p></p></div>';

			echo '<div id="col-right" style="width:50%; float: right"><div style="height:72px;float: left; width:67%"><p>'.
			__('You like this plugin ? Thank you for supporting developer by making a donation. Please rate it on WP plugin directory.', 'simple-catalogue').'</p></div>
		...
		<a href="'. admin_url("admin.php?page=catalogue") .'" class="nav-tab nav-tab-active">'.__('Catalogue Settings', 'simple-catalogue').'</a>';
		// Metabox addon support
		$metabox = 'simple-metabox/simple-metabox.php';
		if ( is_plugin_active($metabox) ) {
		echo '<a href="'. admin_url("admin.php?page=edit-metabox") .'" class="nav-tab">'.__('Metabox Settings', 'simple-catalogue').'</a></h2>';
			}
			else {
			echo '<a href="http://fb-creation.alwaysdata.net/downloads/metabox-addon-pour-simple-catalogue/" class="nav-tab">'.__('Visit the metabox addon page', 'simple-catalogue').'</a>
			</h2>';
			}
			// Start of categories table
		echo '<div class="wrap"><div id="col-right" style="width:50%">
		...
		echo '</table></div></div><div class="wrap">';
	...
	function sc_process_tax()
	{
	    if ( isset( $_POST['submit'] ) ) {
            if (isset($_POST['tax_name']) AND !empty($_POST['tax_name']) AND isset($_POST['tax_names']) AND !empty($_POST['tax_names'])AND isset($_POST['slug']) AND !empty( $_POST['slug'] )) {
                $name = $_POST['tax_name'];
                $slug = $_POST['slug'];
                $slug = htmlspecialchars(mb_strtolower($slug));
